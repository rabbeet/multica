...
func (h *Handler) CreateComment(w http.ResponseWriter, r *http.Request) {
	...
	// a row lock on the issue. The transactional core lives in CommentService
	// so PUL-154's reminder scheduler can commit comments through the same
	// path — DecideFlip and the audit-history invariant must not diverge
	// between callers. Post-commit work (attachments, WS event, agent
	// triggers, response) stays here because it is HTTP-shaped.
	result, err := h.CommentService.Create(r.Context(), service.CommentCreateParams{
		IssueID:     issue.ID,
		WorkspaceID: issue.WorkspaceID,
